...
class aitextgen:
    ...
    :param tf_gpt2: model indicator or directory of OpenAI-distributed version of GPT-2.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # if you already have a local tf model
        elif tf_gpt2 and os.path.isdir(tf_gpt2):
            if not os.path.exists(cache_dir):
                os.makedirs(cache_dir)

            tf_gpt2_safe = re.sub("[^0-9a-zA-Z]+", "_", tf_gpt2)

            if not os.path.isfile(
                os.path.join(cache_dir, f"pytorch_model_{tf_gpt2_safe}.bin")
            ):
                logger.info(
                    f"Converting the {tf_gpt2} GPT-2 TensorFlow weights to PyTorch."
                )

                config_path = os.path.join(tf_gpt2, "hparams.json")

                convert_gpt2_checkpoint_to_pytorch(
                    tf_gpt2, config_path, cache_dir,
                )
                ...
                os.rename(
                    ...
                    os.path.join(cache_dir, f"config_{tf_gpt2_safe}.json"),
                )

            logger.info(f"Loading {tf_gpt2} GPT-2 model from /{cache_dir}.")
            model = os.path.join(cache_dir, f"pytorch_model_{tf_gpt2_safe}.bin")
            config = os.path.join(cache_dir, f"config_{tf_gpt2_safe}.json")
